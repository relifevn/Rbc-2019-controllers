...
def debug(name, val):
  print('[DEBUG] '+ name + ', type = ' + str(type(val)) + ', val= ', end='')
  print(val)
   ...
   # it also ensures that any bytes left over from a previous message are discarded
    ...
    while msg.find("Ready") == -1:
      ...
      while ord(x) != endMarker: # gets the initial debugMessage
        x = ser.read()
        if not (ord(x) == startMarker) and  not (ord(x) == endMarker) and not (ord(x) == specialByte) and not(x == b'\x00'):
...
__comm = "/dev/ttyUSB0"
print("[INFO] Comm = " + __comm)
ser = serial.Serial(__comm, 57600)
...
while n < numLoops:
  ...
  if ser.inWaiting() == 0 and waitingForReply == False:
    ...
    print("BYTES FORM " + bytesToString(teststr))
    print("STRING FORM " + teststr)
    print("===========================")
